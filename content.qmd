...
This document provides a template based on the [quarto system](https://quarto.org/) for contributions to **Computo** @computo. We show how `Python` @perez2011python or  `R`` code can be included. 
...
[LaTeX](https://www.latex-project.org/) code is natively supported [^We use [katex](https://katex.org/) for this purpose.], which makes it possible to use mathematical formulae:
...
It is also posible to cross-reference an equation:

```
Equation `@eq-mylabel` is a more advanced example:
$$
\begin{aligned}
D_{x_N} & = \frac12 
\left[\begin{array}{cc}
...
You can use Jupyter for Python code and more. And R + KnitR for if you want to mix R with Python (via the package reticulate @reticulate).
...
bibliographic references for section, equations, theorem, figure,
listing, etc. HAve a look at [this
